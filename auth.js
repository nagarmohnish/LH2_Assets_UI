...
document.addEventListener('DOMContentLoaded', function () {
    initAuthUI();
    // Show One Tap prompt after 2s if not logged in
    if (!getUser()) {
...
// ======================================
// GOOGLE ONE TAP PROMPT (Demo)
// ======================================

var ONE_TAP_DEMO = {
    name: 'Mohnish Nagar',
    email: 'mohnish.nagar@gmail.com',
    initial: 'M',
    photo: null
};
...
function showOneTap() {
    if (getUser()) return;
    if (document.getElementById('oneTapPrompt')) return;

    var prompt = document.createElement('div');
    prompt.id = 'oneTapPrompt';
    prompt.className = 'onetap';
    prompt.innerHTML =
        '<div class="onetap-inner">' +
            '<button class="onetap-close" id="onetapClose">&times;</button>' +
            '<div class="onetap-header">' +
                '<svg class="onetap-glogo" viewBox="0 0 48 48" width="20" height="20"><path fill="#4285F4" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/><path fill="#34A853" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/><path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/><path fill="#EA4335" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/></svg>' +
                '<span class="onetap-title">Sign in with Google</span>' +
            '</div>' +
            '<div class="onetap-body">' +
                '<div class="onetap-avatar">' + ONE_TAP_DEMO.initial + '</div>' +
                '<div class="onetap-info">' +
                    '<div class="onetap-name">' + ONE_TAP_DEMO.name + '</div>' +
                    '<div class="onetap-email">' + ONE_TAP_DEMO.email + '</div>' +
                '</div>' +
            '</div>' +
            '<button class="onetap-btn" id="onetapContinue">' +
                'Continue as ' + ONE_TAP_DEMO.name.split(' ')[0] +
            '</button>' +
            '<div class="onetap-footer">' +
                'To create an account, Google will share your<br>name, email, and profile picture with this site.' +
            '</div>' +
        '</div>';
    ...
    // Continue button — simulate JWT verification
    document.getElementById('onetapContinue').addEventListener('click', function () {
        var btn = this;
        btn.disabled = true;
        btn.innerHTML = '<span class="onetap-spinner"></span> Signing in...';

        setTimeout(function () {
            var user = {
                name: ONE_TAP_DEMO.name,
                email: ONE_TAP_DEMO.email,
                plan: 'free',
                price: '$0',
                method: 'google_onetap',
                memberSince: new Date().toISOString(),
                avatar: ONE_TAP_DEMO.initial
            };
            setUser(user);
            dismissOneTap();
    ...
    // Close
    document.getElementById('onetapClose').addEventListener('click', function () {
        dismissOneTap();
    });
}

function dismissOneTap() {
    var el = document.getElementById('oneTapPrompt');
    if (!el) return;
